setwd("D:/Projects/Genes_level/Rhizocephala/MolSign_renewed/GSEA_Dmelanogaster/MolSign/")
### Libraries ###
library(topGO)
library(dplyr)
library(rrvgo)
...
stages <- colnames(molsign_tab[, 2:(ncol(molsign_tab)-1)])
...
for (stage in stages){
  ...
  output_file_name <- sprintf("%s_%s_all_genes_GOenrichment_BP_Fisher.min_10_genes.tsv", species_tag, stage)
  ...
  # Visualization #
  # Plot #
  pdf(file=sprintf("%s_%s_all_genes_reduced_GSEA_results.Dmelanogaster_parent_GOterms.pdf", species_tag, stage), width = 10, height = 12)
  result_plot <- stage_reducedTerms_uniq_df %>%
    ...
    ggtitle(sprintf("Reduced GSEA results: \n %s %s molecular signature \n (all genes)", species_tag, stage)) + labs(fill="Log-transformed scores")
  print(result_plot)
  dev.off()
  # ScatterPlot #
  pdf(file=sprintf("%s_%s_all_genes_reduced_GSEA_results.Dmelanogaster.scatterPlot.pdf", species_tag, stage), width = 15, height = 15)
  scatter <- scatterPlot(stage_simMatrix, stage_reducedTerms, size="score", addLabel = TRUE, labelSize = 5)
  print(scatter)
  dev.off()
  # Output writing #
  write.table(stage_reducedTerms, file=sprintf("%s_%s_all_genes_reduced_GSEA_results.Dmelanogaster.tsv", species_tag, stage),
